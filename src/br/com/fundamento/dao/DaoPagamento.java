...
public class DaoPagamento implements IDaoPagamento {
    ...
    public List<Pagamento> buscarpagamento(String data) {
      ...
      public List<Pagamento> buscarpagamentoPeriodo(String data,String date) {
        List<Pagamento> pagamentos = new ArrayList<>();

        int idC = 0, idP = 0;
        try {
            this.conexao = SQLConections.getInstance();
            this.statement = this.conexao.prepareStatement(SQLUtil.Pagamento.buscarpagamentoPeriodo(data,date));
            this.result = this.statement.executeQuery();
            Pagamento pagamento;
            while (result.next()) {
                pagamento = new Pagamento();
       ...
       public List<Pagamento> buscarpagamentoNaoPagoPeriodo(String data,String date) {
        List<Pagamento> pagamentos = new ArrayList<>();

        int idC = 0, idP = 0;
        try {
            this.conexao = SQLConections.getInstance();
            this.statement = this.conexao.prepareStatement(SQLUtil.Pagamento.buscarpagamentoNaoPagoPeriodo(data,date));
            this.result = this.statement.executeQuery();
            Pagamento pagamento;
            while (result.next()) {
                pagamento = new Pagamento();
